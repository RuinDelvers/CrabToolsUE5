...
class FEventSlotStyleSet final : public FSlateStyleSet
{
public:
	...
	FEventSlotStyleSet(const FName& InStyleSetName)
		: FSlateStyleSet(InStyleSetName)
	{
		...
		Set("EventSlot.ChipButton.Selected", ChipButtonSelected);
		...
		Set("EventSlot.ChipButton.Unselected", ChipButtonUnselected);
		...
		Set("EventSlot.ChipClearButton", ChipClearButton);

		Set("EventSlot.Container", new FSlateRoundedBoxBrush(FStyleColors::Input, 4.0f, FStyleColors::InputOutline, 1.0f));
		...
		Set("EventSlot.ComboButton", TagComboButton);
...
TSharedPtr<FSlateStyleSet> FEventSlotStyle::StyleSet = nullptr;


void FEventSlotStyle::Initialize()
{
	if (StyleSet.IsValid())
	{
		return;
	...
	StyleSet = MakeShared<FEventSlotStyleSet>(GetStyleSetName());
	
	FSlateStyleRegistry::RegisterSlateStyle(*StyleSet.Get());
}

void FEventSlotStyle::Shutdown()
{
	if (StyleSet.IsValid())
	{
...
FName FEventSlotStyle::GetStyleSetName()
{
	static FName StyleName("EventSlotStyle");
	return StyleName;
}
